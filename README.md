...
阅读顶部“另存为 MD”与聊天菜单共用纯文本导出程序。阅读更多菜单同步官方文件入口、置顶、删除和移至项目；原有归档项继续移除。阅读输入区不改动；消息操作与书签合并列入 `TODO.md`。
...
验收入口：`node scripts/test-project-export.mjs`（隔离目录、Python zipfile 独立校验 ZIP/CRC/UTF-8）与 `node scripts/test-reader-ui-browser.mjs`（开发与 `CEOBE_TEST_DIST=1` 生产模式；原生保存接口桩、取消/回退、菜单/设置非写入预览、图片比例/键盘/遮罩）。实际 Windows 保存窗口仍由用户最终验收。
...
完整项目设置、记忆说明和图标选择器已同步为显式预览：不保存指令、AI 记忆、库访问或图标变更；名称通过侧栏重命名修改，设置中的删除按钮进入既有保留聊天确认。先验收界面，再决定裁剪，不启用云端共享。
...
“最近”标题栏使用冻结的官方展开/收起 DOM，右侧撰写入口进入导入页。“整理聊天”菜单支持按列表或按项目：按项目时显示项目分组及未归属聊天；项目可展开实际关联会话，空项目使用官方“暂无项目聊天”占位。项目行右侧编辑图标进入 `project.html?id=…`；项目更多支持本地重命名、置顶和保留聊天的项目删除，分享改为 ZIP，完整设置只预览、不写入。置顶区独立保留，置顶会话也可从其所属项目访问。
