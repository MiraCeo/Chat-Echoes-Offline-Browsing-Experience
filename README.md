...
- **整理成资料**：搜索聊天标题、置顶常用对话，使用项目分组和本地项目简介组织内容。
...
- **数据保存在本地工作区。** 聊天与附件位于 `archive/`，项目和聊天显示元数据位于 `data/private/`。请为重要资料自行备份，详见[数据与备份](docs/usage.md#数据与备份)。
...
- **两种删除含义不同。** 删除项目只解除项目及关联，保留聊天、附件和历史归档；删除聊天会永久删除该聊天的全部归档版本及相应派生页面。
- **导出不等于备份。** MD 和项目 ZIP 是纯文本导出，不包含附件或项目简介，也不能代替完整归档备份。
...
- 纯静态部署提供阅读快照；导入、修改项目和即时 MD／ZIP 导出需要本地服务。
- 已有项目的图标／颜色编辑、项目来源管理及部分消息操作尚未接入。
