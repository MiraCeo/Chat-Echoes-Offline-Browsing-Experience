...
侧栏项目行及项目详情页的更多按钮复用 `project-more.json` 中冻结的官方主菜单，包含分享项目、重命名项目、项目设置、项目主页、置顶项目和删除项目。项目主页直接跳转；重命名、置顶/取消置顶和删除已接入本地元数据。侧栏重命名使用捕获的官方行内输入，详情页使用官方项目名称表单的精简版本（回车保存，Esc/关闭取消）；删除使用独立的官方项目确认结构，明确保留所有聊天、附件和历史归档，只删除项目及关联。置顶项目排在项目列表前面。分享与完整项目设置仍不提交写入；不会误用聊天永久删除弹窗。
...
“最近”标题栏使用冻结的官方展开/收起 DOM，右侧撰写入口进入导入页。“整理聊天”菜单支持按列表或按项目：按项目时显示项目分组及未归属聊天；项目可展开实际关联会话，空项目使用官方“暂无项目聊天”占位。项目行右侧编辑图标进入 `project.html?id=…`；项目更多支持本地重命名、置顶和保留聊天的项目删除，未确定的分享/完整设置不接入。置顶区独立保留，置顶会话也可从其所属项目访问。
...
- **开发服务**：`npm run dev`，支持导入、聊天/项目管理与 MD 转存。
