...
export function markdownExportMiddleware(root){const store=createChatStore(root);return async(req,res,next)=>{
 ...
 try{
  ...
  if(!inside(project,archive)||!inside(archive,folder)||!inside(folder,file))return send(409,{error:'归档路径异常，已拒绝导出'});
  const conversation=JSON.parse(await readFile(file,'utf8'));conversation.title=entry.title;
  return send(200,{title:entry.title,filename:markdownFilename(entry.title),markdown:exportConversationMarkdown(conversation,{textOnly:true})});
 }catch(e){console.error('Markdown export:',e.message);return send(e.code==='ENOENT'?404:500,{error:e.code==='ENOENT'?'聊天归档不存在或已删除':'转存失败，请检查本地归档和服务日志'})}
}}
