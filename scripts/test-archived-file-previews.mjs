...
const previews = [...document.querySelectorAll('template[data-ceobe-preview]')];
assert.equal(previews.length, 4);

const parsed = previews.map(template => ({
  key: template.getAttribute('data-ceobe-preview'),
  document: parseHTML(`<html><body>${template.innerHTML}</body></html>`).document,
}));
...
for (const image of pdf.document.querySelectorAll('[data-testid^="artifact-pdf-page-"] img')) {
  await access(resolve('replay/conversations', image.getAttribute('src')));
}
assert.ok(docx.document.querySelector('[data-testid="docx-preview-panel"] p'));
