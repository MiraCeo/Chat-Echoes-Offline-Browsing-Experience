...
import{createBookmarkStore}from'./bookmark-store.mjs';import{bookmarkTargets}from'./bookmark-targets.mjs';
...
try{
 ...
 const bookmarks=createBookmarkStore(root);for(const id of ['chat-one','chat-two'])await bookmarks.mutate({action:'save',chatId:id,messageId:bookmarkTargets(sample)[0].message_id,title:'个人书签',note:'PRIVATE-BOOKMARK-NOTE-'+id,expectedVersion:0});const bookmarkBefore=await readFile(join(root,'data/private/bookmarks.ceobe.json'));
 ...
 const failed=createChatStore(root,{rebuild:async()=>{throw new Error('injected build failure')}});await assert.rejects(()=>failed.mutate({id:'chat-one',action:'delete',confirm:'chat-one'}));assert.equal(await readFile(join(root,'replay/old-secret.html'),'utf8'),'RAW-SECRET-chat-one');assert.deepEqual(await readFile(join(root,'data/private/bookmarks.ceobe.json')),bookmarkBefore);
 ...
 assert.deepEqual((await bookmarks.list()).map(b=>b.chat_id),['chat-two']);for(const dir of ['replay','dist'])assert.ok(!(await readFile(join(root,dir,'index.html'),'utf8')).includes('PRIVATE-BOOKMARK-NOTE-'));
 ...
 await store.mutate({id:'chat-two',action:'delete',confirm:'chat-two'});assert.equal((await store.list()).length,0);assert.equal((await bookmarks.list()).length,0);assert.ok((await readFile(join(root,'dist/index.html'),'utf8')).includes('本地聊天'));
