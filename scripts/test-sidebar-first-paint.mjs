...
const pages = ['index.html', 'import.html', 'assets.html', ...(await readdir('replay/conversations')).filter(n => n.endsWith('.html')).map(n => `conversations/${n}`)];
...
try {
  ...
  for (const path of ['index.html', 'import.html', 'assets.html', pages.at(-1)]) {
