...
export function zipTextFiles(files){
 ...
 for(const {name,text} of files){if(!name||/[\\/\x00]/.test(name)||name==='.'||name==='..')throw Error('无效 ZIP 文件名');const filename=Buffer.from(name,'utf8'),data=Buffer.from(text,'utf8');total+=data.length;if(total>200*1024*1024)throw Error('项目文本超过 200 MiB，请分批导出');const compressed=deflateRawSync(data),crc=crc32(data);if(filename.length>65535)throw Error('ZIP 文件名过长');
